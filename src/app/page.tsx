...
export default function LandingPage() {
  ...
  return (
    <div className="overflow-x-hidden">
      {/* NAVBAR */}
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-[#0F172A]/95 backdrop-blur-xl border-b border-[#1D4ED8]/10 shadow-lg shadow-black/20' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <Logo />
          <nav className="hidden md:flex items-center gap-8">
            {['Home','Tentang','Laboratorium','Cara Peminjaman'].map(item => (
              <a key={item} href={`#${item.toLowerCase().replace(/ /g,'-')}`} className="text-sm text-[#DBEAFE]/75 hover:text-[#F8FAFC] transition-colors" style={{fontFamily:BODY}}>{item}</a>
            ))}
          </nav>
          <div className="flex items-center gap-3">
            <Link href="/login" className="hidden md:flex items-center gap-2 px-5 py-2 rounded bg-[#1D4ED8] hover:bg-[#3B82F6] text-[#0F172A] text-sm font-semibold transition-all hover:shadow-[0_0_24px_rgba(29,78,216,0.45)]" style={{fontFamily:BODY}}>Login</Link>
        ...
        {mobileMenu && (
          <div className="md:hidden bg-[#1E293B] border-t border-[#1D4ED8]/10 px-6 py-5 flex flex-col gap-4">
            {['Home','Tentang','Laboratorium','Cara Peminjaman'].map(item => (
            ...
            <Link href="/login" className="mt-1 py-2.5 rounded bg-[#1D4ED8] text-[#0F172A] text-sm font-semibold text-center" style={{fontFamily:BODY}}>Login</Link>
          </div>
        )}
      </header>
      ...
      <section id="home" className="relative min-h-screen flex flex-col overflow-hidden" style={gridBg}>
        <div className="absolute inset-0 pointer-events-none" style={{background:'radial-gradient(ellipse 65% 55% at 18% 55%,rgba(29,78,216,0.10) 0%,transparent 65%)'}}/>
        <div className="relative flex-1 flex items-center max-w-7xl mx-auto px-6 w-full pt-20 pb-8">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-12 xl:gap-20 items-center w-full">
            <div>
              <motion.div initial={{opacity:0,x:-24}} animate={{opacity:1,x:0}} transition={{delay:0.1}} className="inline-flex items-center gap-2.5 mb-8 px-3.5 py-1.5 rounded border border-[#1D4ED8]/20 bg-[#1D4ED8]/5">
                <PulseDot/>
                <span className="text-[#F8FAFC] text-[11px] font-medium tracking-[0.14em] uppercase" style={{fontFamily:MONO}}>Teknik Industri · Universitas Sultan Ageng Tirtayasa</span>
              </motion.div>
              <div style={{fontFamily:DISPLAY}} className="leading-[0.9] mb-7">
                ...
                <motion.div initial={{opacity:0,y:40}} animate={{opacity:1,y:0}} transition={{delay:0.42,duration:0.55,ease:[0.22,1,0.36,1]}} className="text-[56px] md:text-[72px] lg:text-[88px] font-black text-[#F8FAFC] tracking-[-0.01em]">LABORATORIUM</motion.div>
              </div>
              <motion.p initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.52}} className="text-[#DBEAFE]/90 text-[15px] leading-relaxed mb-9 max-w-[480px]" style={{fontFamily:BODY}}>Platform terintegrasi untuk peminjaman, monitoring, dan pengelolaan alat laboratorium secara real-time. Dirancang khusus untuk mahasiswa dan asisten lab Teknik Industri UNTIRTA.</motion.p>
              <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.62}} className="flex items-center gap-4 flex-wrap">
                <Link href="/login" className="group flex items-center gap-2.5 px-7 py-3.5 rounded bg-[#1D4ED8] hover:bg-[#3B82F6] text-[#0F172A] font-semibold text-sm transition-all hover:shadow-[0_0_36px_rgba(29,78,216,0.5)]" style={{fontFamily:BODY}}>Pinjam Alat <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform"/></Link>
                <Link href="/login" className="group flex items-center gap-2.5 px-7 py-3.5 rounded border border-slate-700 hover:border-[#1D4ED8]/40 text-slate-300 hover:text-white font-medium text-sm transition-all" style={{fontFamily:BODY}}><Eye className="w-4 h-4 text-slate-500 group-hover:text-[#F8FAFC] transition-colors"/>Lihat Inventaris</Link>
              </motion.div>
            </div>
            {/* Live Panel */}
            <motion.div initial={{opacity:0,x:48}} animate={{opacity:1,x:0}} transition={{delay:0.45,duration:0.65}} className="hidden lg:block">
              <div className="relative p-5 rounded-xl border border-[#1D4ED8]/15 bg-[#1E293B]/90 backdrop-blur-sm shadow-2xl shadow-black/40">
                <Brackets/>
                <div className="flex items-center justify-between mb-5 pb-4 border-b border-white/5">
                  <div className="flex items-center gap-2"><PulseDot/><span className="text-[#F8FAFC] text-[11px] tracking-[0.14em] uppercase" style={{fontFamily:MONO}}>LIVE · SISTEM MONITOR</span></div>
                  <span className="text-[#BFDBFE]/55 text-[10px]" style={{fontFamily:MONO}}>SYS_v2.4.1</span>
                </div>
                <div className="grid grid-cols-3 gap-3 mb-5">
                  {[{label:'Total Alat',val:alat,icon:Package,color:'text-[#F8FAFC]'},{label:'Laboratorium',val:labs,icon:Layers,color:'text-[#F8FAFC]'},{label:'Pengguna',val:users,icon:Users,color:'text-[#93C5FD]'}].map(({label,val,icon:Icon,color})=>(
                    <div key={label} className="p-3 rounded-lg bg-[#0F172A] border border-white/5">
                      ...
                      <span className="text-[#BFDBFE]/70 text-[11px]" style={{fontFamily:BODY}}>{label}</span>
                    </div>
                  ))}
                </div>
                <div>
                  <div className="flex items-center justify-between mb-3"><span className="text-slate-500 text-[11px] uppercase tracking-wider" style={{fontFamily:MONO}}>Peminjaman Aktif</span><span className="text-[11px] px-2 py-0.5 rounded bg-[#1D4ED8]/10 text-[#F8FAFC] font-medium" style={{fontFamily:MONO}}>{activeLoans.length} AKTIF</span></div>
                  <div className="space-y-2">
                    {activeLoans.length > 0 ? activeLoans.map((item, i) => (
                      <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-[#0F172A] border border-white/5 hover:border-[#1D4ED8]/15 transition-colors">
                        <div className="min-w-0"><div className="text-white text-[13px] font-medium truncate mb-0.5" style={{fontFamily:BODY}}>{item.name}</div><div className="text-[#BFDBFE]/65 text-[11px]" style={{fontFamily:MONO}}>{item.lab} · {item.user}</div></div>
                        <span className={`ml-3 text-[11px] px-2 py-0.5 rounded font-medium flex-shrink-0 ${
                          item.status === 'dipinjam' ? 'bg-[#1D4ED8]/10 text-[#F8FAFC]' :
                      ...
                      <div className="p-4 text-center border border-white/5 rounded-lg bg-[#0F172A]/50">
                        <span className="text-[#BFDBFE]/65 text-[11px]" style={{fontFamily:MONO}}>TIDAK ADA PEMINJAMAN</span>
                      </div>
                    )}
                  </div>
                </div>
        ...
        {/* Bottom Strip */}
        <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:0.78}} className="relative border-t border-white/5 bg-[#0F172A]/60">
          <div className="max-w-7xl mx-auto px-6 py-5 grid grid-cols-2 md:grid-cols-4 gap-6">
            {[{label:'Status Alat',val:'Real-time',icon:Activity},{label:'Uptime Sistem',val:'24/7',icon:Server},{label:'Akurasi Data',val:'100%',icon:Crosshair},{label:'Role Akses',val:'Multi-Role',icon:Shield}].map(({label,val,icon:Icon})=>(
              <div key={label} className="flex items-center gap-3 group">
                <div className="w-9 h-9 rounded-lg bg-[#1D4ED8]/5 border border-[#1D4ED8]/15 flex items-center justify-center flex-shrink-0 group-hover:border-[#1D4ED8]/35 transition-colors"><Icon className="w-4 h-4 text-[#F8FAFC]"/></div>
                <div><div className="text-white font-semibold text-sm" style={{fontFamily:BODY}}>{val}</div><div className="text-[#BFDBFE]/70 text-xs" style={{fontFamily:BODY}}>{label}</div></div>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      <section id="tentang" className="bg-white py-28 relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6">
          <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div>
              <div className="flex items-center gap-3 mb-5"><div className="h-px w-10 bg-[#1D4ED8]"/><span className="text-[#1E3A8A] text-[11px] font-medium tracking-[0.22em] uppercase" style={{fontFamily:MONO}}>Tentang LabTrack</span></div>
              <h2 className="text-[48px] md:text-[64px] font-black text-slate-900 leading-[0.92] tracking-tight" style={{fontFamily:DISPLAY}}>KENAPA HARUS<br/><span className="text-[#1E3A8A]">LABTRACK?</span></h2>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {[
              {icon:Activity,title:'Real-time Monitoring',desc:'Status alat dan peminjaman terupdate secara real-time tanpa perlu refresh manual.',badge:'01',accent:'text-[#1E3A8A]',bg:'bg-[#EFF6FF]',bar:'bg-[#1D4ED8]'},
              {icon:Shield,title:'Aman & Terstruktur',desc:'Data peminjaman tersimpan rapi, teraudit, dan terlindungi dengan sistem validasi berlapis.',badge:'02',accent:'text-[#0F172A]',bg:'bg-[#EFF6FF]',bar:'bg-[#1E3A8A]'},
              {icon:BarChart3,title:'Dashboard Monitoring',desc:'Visualisasi data peminjaman, inventaris, dan aktivitas laboratorium dalam satu tampilan.',badge:'03',accent:'text-[#1E40AF]',bg:'bg-[#EFF6FF]',bar:'bg-[#1E40AF]'},
              {icon:Users,title:'Multi-Role Access',desc:'Akses berbeda untuk mahasiswa dan asisten lab — setiap peran melihat fungsi yang relevan.',badge:'04',accent:'text-[#0F172A]',bg:'bg-[#EFF6FF]',bar:'bg-[#0F172A]'},
            ].map((f,i)=>(
      ...
      <section id="laboratorium" className="bg-slate-50 py-28">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <h2 className="text-[48px] md:text-[64px] font-black text-slate-900 leading-[0.92] tracking-tight mb-5" style={{fontFamily:DISPLAY}}>4 LABORATORIUM<br/><span className="text-[#1E3A8A]">TEKNIK INDUSTRI</span></h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {[
              ...
              <motion.div key={lab.id} initial={{opacity:0,scale:0.96}} whileInView={{opacity:1,scale:1}} viewport={{once:true}} transition={{delay:i*0.08}} whileHover={{y:-4}} className="group relative p-6 rounded-xl border border-slate-200 bg-white hover:border-[#1D4ED8]/60 transition-all duration-300 cursor-pointer overflow-hidden">
                <div className="flex items-start gap-5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-[11px] font-semibold text-[#1E3A8A] uppercase tracking-widest" style={{fontFamily:MONO}}>{lab.id}</span>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1 text-slate-400 group-hover:text-[#1E3A8A] transition-colors"><span className="text-[12px] font-medium" style={{fontFamily:BODY}}>Lihat Alat</span><ChevronRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform"/></div>
                    </div>
                  </div>
                </div>
              </motion.div>
      ...
      <section id="cara-peminjaman" className="py-28 relative overflow-hidden" style={gridBg}>
        <div className="absolute inset-0 pointer-events-none" style={{background:'radial-gradient(ellipse 90% 60% at 50% 50%,rgba(29,78,216,0.06) 0%,transparent 70%)'}}/>
        <div className="relative max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <div className="flex items-center justify-center gap-4 mb-5"><div className="h-px w-12 bg-gradient-to-r from-transparent to-[#1D4ED8]"/><span className="text-[#F8FAFC] text-[11px] tracking-[0.22em] uppercase" style={{fontFamily:MONO}}>Cara Peminjaman</span><div className="h-px w-12 bg-gradient-to-l from-transparent to-[#1D4ED8]"/></div>
            <h2 className="text-[48px] md:text-[64px] font-black text-white leading-[0.92] tracking-tight" style={{fontFamily:DISPLAY}}>EMPAT LANGKAH<br/><span className="text-[#F8FAFC]">MUDAH & CEPAT</span></h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5 relative">
            <div className="hidden md:block absolute top-[52px] left-[12.5%] right-[12.5%] h-px"><div className="w-full h-full bg-gradient-to-r from-[#1D4ED8]/20 via-[#F8FAFC]/40 to-[#1D4ED8]/20"/></div>
            {[
              ...
              <motion.div key={step.num} initial={{opacity:0,y:36}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*0.13,duration:0.5}}>
                <div className="group relative p-6 rounded-xl border border-[#1D4ED8]/15 bg-[#1E293B] hover:border-[#1D4ED8]/35 transition-all duration-300 h-full">
                  <Brackets/>
                  <div className="flex items-start justify-between mb-5">
                    <div className="w-11 h-11 rounded-lg bg-[#1D4ED8]/10 border border-[#1D4ED8]/20 flex items-center justify-center group-hover:bg-[#1D4ED8]/20 transition-colors"><step.icon className="w-5 h-5 text-[#F8FAFC]"/></div>
                    <span className="text-4xl font-black text-[#1E3A8A]/15 leading-none" style={{fontFamily:DISPLAY}}>{step.num}</span>
                  </div>
      ...
      <section className="py-28 relative overflow-hidden bg-white">
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[#EFF6FF] rounded-full blur-[100px] opacity-60 pointer-events-none"/>
      ...
      {/* FOOTER */}
      <footer className="border-t border-white/5 py-14" style={{background:'#0F172A'}}>
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-5">
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li className="flex items-start gap-3">
                <div className="text-[#1E3A8A] mt-0.5"><Mail className="w-4 h-4"/></div>
                <div><div className="text-slate-400 text-[11px] mb-0.5" style={{fontFamily:MONO}}>EMAIL</div><a href="mailto:3333230025@untirta.ac.id" className="text-slate-300 hover:text-[#F8FAFC] transition-colors text-sm font-medium" style={{fontFamily:BODY}}>3333230025@untirta.ac.id</a></div>
              </li>
              <li className="flex items-start gap-3">
                <div className="text-[#1E3A8A] mt-0.5"><Phone className="w-4 h-4"/></div>
                <div><div className="text-slate-400 text-[11px] mb-0.5" style={{fontFamily:MONO}}>WHATSAPP</div><a href="https://wa.me/6283806241350" target="_blank" rel="noreferrer" className="text-slate-300 hover:text-[#F8FAFC] transition-colors text-sm font-medium" style={{fontFamily:BODY}}>+62 838-0624-1350</a></div>
              </li>
              <li className="flex items-start gap-3">
                <div className="text-[#1E3A8A] mt-0.5"><MapPin className="w-4 h-4"/></div>
          ...
          <div className="flex flex-col items-start md:items-end justify-between">
            <div className="flex items-center gap-2"><PulseDot active={true}/><span className="text-[#F8FAFC] text-[11px] tracking-widest uppercase" style={{fontFamily:MONO}}>Sistem Online</span></div>
            <p className="text-[#BFDBFE]/70 text-sm text-left md:text-right mt-10 md:mt-0" style={{fontFamily:BODY}}>© 2026 LabTrack<br/>Teknik Industri, Universitas Sultan Ageng Tirtayasa</p>
          </div>
        </div>
      </footer>
    </div>
